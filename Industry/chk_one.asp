...
sub zs_sc()
	...
	call CreateRs(rs,"select * from 不良计算 where 部番='"&request.cookies("bf")&"' and 时间='"&date()&"'",1,3)
	select case bottn
	case "a"
		rs("黑点")=rs("黑点")+1
	case "b"
...
'********************************
sub zs_js()
	call CreateRs(rs,"select * from 不良计算 where 时间='"&date()&"' and 机台='"&request.cookies("scjt")&"'",1,3)
...
sub zs_sctj()                 
	...
	call CreateRs(rs,"select * from 不良计算 where 部番='"&request.cookies("bf")&"' and 机台='"&request.cookies("scjt")&"' and 时间='"&date()&"'",1,1)	
	if rs("黑点")>0 then
	...
	call closeRs(rs)
	 '初始化计划白板当前部品完成状态                                     
	call CreateRs(rs,"select * from 计划白板 where equip_name='"&request.cookies("scjt")&"' and goods_name='"&request.cookies("bf")&"'",1,3)
	rs("state")="Y"
	rs.update
	call closeRs(rs)	
	'计算机台停机时间计算
...
'*************生产数量写入作业日报****************
	'不良总数&产量统计
	call CreateRs(rs,"select * from 不良计算 where 部番='"&request.cookies("bf")&"' and 时间='"&date()&"' and 机台='"&request.cookies("scjt")&"'",1,1) 
	...
	call closeRs(rs)
	'间断时间总数
	call CreateRs(rs,"select * from 停机时间 where 部番='"&request.cookies("bf")&"' and 作业日期='"&date()&"' and 机台='"&request.cookies("scjt")&"'",1,1)
	if not rs.eof then
	dim zdhj
	zdhj=0
	do while not rs.eof
	...
	call closeRs(rs)
	'不良计算初始化
	call CreateRs(rs,"select * from 不良计算 where 部番='"&request.cookies("bf")&"' and 时间='"&date()&"' and 机台='"&request.cookies("scjt")&"'",1,3)  
	do while not rs.eof
	rs.delete
	rs.update
	rs.movenext
