...
<title>HUDSON工业管理系统--380T-1机台状态</title>
<link href="../css/css.css" rel="stylesheet" type="text/css" />
	<% 
	dim bf,jhs,start_time,zq,s_zq,zzy,bb,scsl,qs,fs,s_overtime,scxl,ss,state_time
	call CreateRs(rs,"select * from equip where equip_name='380T-1'",1,1)
	...
	call closeRs(rs)
	call CreateRs(rs,"select * from 作业日报 where 机台='380T-1' and work_state='Z'",1,1)
	if not rs.eof then	
	bf=rs("加工部番")
	jhs=rs("计划数量")
	start_time=rs("起始时间")
