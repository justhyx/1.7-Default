...
<form id="form1" name="form1" method="post" action="">
  <table width="95%" border="0" align="center" cellpadding="4" cellspacing="1">
    <tr>
	  ...
	  <td align="right">	  	  <input type="button" name="Submit222" onclick="javascript:window.location.href='itevent.asp';" style="font-weight:bold; height:30px; width:120px;" value=" IT事务申请 " /></td>
      <td width="227">&nbsp;</td>
    </tr>
  </table>
</form>
...
<table border="0" style="margin-left:30px;" cellpadding="4" cellspacing="1" bordercolor="#000000" width="95%" bgcolor="#F1F1F1" id="mytable">
  <tr>
    <td colspan="7" align="center" style=" font-weight:bold;">IT依赖事务申请查询&nbsp;</td>
  </tr>
...
If TxtKey="" Then
	call CreateRs(rs,"select * from 电脑维护记录 where 状态='已处理' Order By 维护日期 DESC",1,1)
		if rs.eof or rs.bof then
			response.End()
		end if
Else
	call CreateRs(rs,"select * from 电脑维护记录 Where comuser like '%"&TxtKey&"%' and 状态='未处理' Order By 维护日期 DESC",1,1)
		if rs.eof or rs.bof then
...
%>  
  <tr align="center">
    ...
    <td><%= rs("维护项目") %>&nbsp;</td>
    <td><%= rs("现象描述") %>&nbsp;</td>
    <td width="60"><%= rs("状态")  %>&nbsp;</td>
	<% if session("user_l")="11" then%>
	<td width="60"><a href="queryIT.asp?id=<%= rs("id")%>">明细查看</a>&nbsp;</td>	
	<% end if%>
  </tr>
    <%
	i=i+1
