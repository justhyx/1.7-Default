...
<body>
<form id="form1" name="form1" method="post" action="other_chk.asp?action=<%= gudge %>&id=<%= id %>">
  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#000000">
    <tr>
      <td colspan="4" align="left" style="font-weight:bold; background-color:#FF0000; color:#FFFFFF;">预算外加班申请填写</td>
    </tr>
    <tr>
      <td>部门</td>
      <td>开始时间</td>
    ...
    <tr>
      ...
      <td><label>
        <input name="note" type="text" id="note" value="<%= why%>" size="50" />
      </label></td>
    </tr>
    <tr>
...
<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#000000">
  ...
  <% 
  	call CreateRs(rs,"select * from overtime where username='"&session("UserName")&"' and mm='"&month(date)&"' and other_flog='"&int(1)&"' and manager_ok='"&int(1)&"'",1,1)
	if rs.eof then 
	response.End()
	end if
	is_week=0
  ...
  <tr>
    ...
    <td><%=rs("noweek")  %></td>
    <td><%=rs("isweek")  %></td>
    <td><a href="other_add.asp?op=modi&id=<%= rs("id") %>">修改</a> | <a href="other_chk.asp?action=dell&amp;id=<%= rs("id") %>">取消</a></td>
