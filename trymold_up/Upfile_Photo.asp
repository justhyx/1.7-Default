...
sub upload_0()    '使用化境无组件上传类
	...
	for each formName in upload.file '列出所有上传了的文件
		...
		if FoundErr<>true then
			...
			goods_name=	Trim(upload.Form("goods_name"))
			
				call CreateRs(rs,"select * from trymold_up where goods_name='"& goods_name &"'",1,3)
				if rs.eof then
				rs.addnew
				rs("goods_name")=goods_name				
				rs("remark")=Trim(upload.Form("remark"))
				rs("pic_address")="UploadFiles\"&year(now)&month(now)&day(now)&hour(now)&minute(now)&second(now)&ranNum&"."&fileExt	
				rs.update
				else
				conn.execute("update trymold_up set pic_address='"& filename &"' where goods_name='"& goods_name &"'")
				end if
