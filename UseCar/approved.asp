...
If TxtKey="" Then
	call CreateRs(rs,"select * from UseCar where delfolg<>'"&int(2)&"' Order By id DESC ",1,1)
		if rs.eof or rs.bof then
			response.End()
		end if
Else
	call CreateRs(rs,"select * from UseMacAddress Where UseName like '%"&TxtKey&"%' over_l='"& session("over_l") &"' Order By ID DESC",1,1)
		if rs.eof or rs.bof then
