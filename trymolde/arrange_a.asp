...
dd=getFormatDateString(day(now))
dastr=yy&"-"&mm&"-"&dd
%>
</head>
...
<body>
          <form id="form1" name="form1" method="post" action="">
            ...
            <label class="noprint">输入日期查询 例：2012-06-07 注：输入填单日期</label>
          </form>
...
%>
	<form id="form<%= i %>" name="form<%= i %>" method="post" action="chk.asp?action=arrange_a">
    <tr>
      <td><%= rs("试模类型") %> | <%= rs("类型") %></td>
      <td><%= rs("品番") %>&nbsp;</td>
      <td><%= rs("生准要望机台") %>&nbsp;</td>
      ...
      <td><%= rs("材料") %></td>
      <td><%= rs("备注") %></td>
      <td><label><%= rs("addtime") %></label></td>
      <td><%= rs("材料编号") %></td>
      <td><label><%= rs("模具担当") %></label></td>
    </tr>
	</form>
    <%
		i=i+1
